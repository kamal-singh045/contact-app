...
import { contactInitialState, ContactsContext } from "../contexts/ContactsContext";
...
const FormModal: React.FC<DataType> = ({ setOpenModal }) => {
    ...
    const handleFormSubmit = async (data: ContactDataType) => {
        try {
            isEditing ? updateContact(data as ContactDataType) : addContact(data as ContactDataType);
            setOpenModal(false);
            setIsEditing(false);
    ...
    const { addContact, updateContact, isEditing, setIsEditing, currentContact, setCurrentContact, methods } = useContext(ContactsContext) as ContactsContextType;
    ...
    return (
        <div ref={clickRef} className={`rounded-md fixed top-[50%] left-[50%] -translate-x-2/4 -translate-y-2/4 bg-[#d6bdf6] w-[97vw] md:w-[90vw] 2xl:w-[60vw] min-h-[20rem] p-4 md:p-8 z-10`}>
            ...
            <FormProvider {...methods}>
                <form onSubmit={methods.handleSubmit(handleFormSubmit)} className="mt-8 flex flex-col gap-8">
                    ...
                    <div className="grid grid-cols-[6rem_auto] gap-4 items-center">Role:
                        <SelectComponent
                            name="role"
                            options={Object.values(RoleEnum).filter(role => role !== RoleEnum.ALL)}
