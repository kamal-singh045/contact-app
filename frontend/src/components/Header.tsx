...
const Header = () => {
  const { setRoleFilter, openModal, setOpenModal } = useContext(
    ContactsContext,
  ...
  return (
    <div className="flex overflow-scroll gap-2 md:gap-4 mx-1 sm:mx-4 mt-6 bg-white rounded-md p-4">
      <button
        ...
        className="rounded-full shrink-0 px-6 py-1 border border-black"
      >
        Add
      </button>
      <FormProvider {...headerMethods}>
        <form onChange={headerMethods.handleSubmit(roleChangeHandler)}>
          <SelectComponent
            ...
            customClasses="border border-black rounded-full"
          />
        </form>
      </FormProvider>
      <Searchbar />
