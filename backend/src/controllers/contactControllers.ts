...
class ContactControllers {
  addContactController: RequestHandler = async (req, res, next) => {
    try {
      const validatedData = await AddContactSchema.validate(req.body).catch(
        (err) => err,
      );
      if (validatedData.errors && validatedData.errors.length)
        return res
          .status(StatusCodeEnum.FORBIDDEN)
          .send({ status: false, message: validatedData.errors[0] });

      const alreadyExists = await contactsModel.findOne({
        email: req.body.email,
      });
      if (
        (!req.body._id && alreadyExists) ||
        (alreadyExists &&
          (alreadyExists?.email !== req.body.email ||
            (alreadyExists?.email === req.body.email &&
              alreadyExists._id.toString() != req.body._id)))
      )
        return res
      ...
      await contactsModel.findByIdAndUpdate({ _id: id }, req.body, {
        new: true,
        upsert: true,
      });
      return res
        .status(StatusCodeEnum.CREATED)
        .send({ status: true, message: "Contact is Created" });
    } catch (error) {
      console.log(error);
      next();
    }
    ...
    try {
      ...
      if (!checkExists)
        return res
          ...
          .send({ status: false, message: "Contact does not exists" });
  ...
  getAllContacts: RequestHandler = async (req, res, next) => {
    try {
      ...
      const allData = await contactsModel
        .find(filterObj)
